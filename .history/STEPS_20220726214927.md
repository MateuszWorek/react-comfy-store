...
## REMOVE ITEM AND CART ITEM FUNCTIONALITY
...
## USER AUTHENTICATION - AUTH0 SETUP
...
```js
REACT_APP_AUTH_DOMAIN=dev--kljksrb.us.auth0.com
REACT_APP_AUTH_CLIENT_ID=XAgCo1dWFyA5JmcR2my5pJcYd6eAxILn
REACT_APP_STRIPE_PUBLIC_KEY=pk_test_51LP90aBR9zbbDPOELZOxowJOwSTxizK5v8P4dKCTYbNv3sfj9KhGtIzSlkRJ8gimCe67mdkRovgCg6A1unckJpK600ZfqMWE6I
REACT_APP_STRIPE_SECRET_KEY=sk_test_51LP90aBR9zbbDPOEN0SjaidZLIerB2LuUqQjqkfAhZtcJWBtcJVclwutNgPz3RGND4THL1Z9aUbKl4IgSyaf4sh000PqHOcEGN
```
...
// Wrap logic in Elements wrapper
...
const CheckoutForm = () => {
  ...
  const createPaymentIntent = async () => {
    console.log('hello from StripeCheckput');
  };
...
- :root/functions/create-payment-intent.js
...
- components/ShippingCheckout.js
...
```js
const createPaymentIntent = async () => {
  // console.log('hello from StripeCheckout');
  try {
...
```js
{
  succeeded ? (
    ...
    <article>
      ...
      <p>Test Cad Number: 4242 4242 4242 4242</p>
    </article>
  );
}
<form id='payment-form' onSubmit={handleSubmit}>
  ...
  {/* Show any error thet happens when processing the payment */}
  {error && (
  ...
  <p className={succeeded ? 'resilt-message' : 'result-message hidden'}>
...
```txt
git remote add origin https://github.com/MateuszWorek/react-comfy-store.git
git branch -M main
